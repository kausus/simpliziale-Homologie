...
\begin{Bsp}
  ...
  \item Sei $\sigma^n \coloneqq e_0 \ldots e_n $ der Standardsimplex.
    ...
    Simplizialkomplex, schreibe hierfür $\Delta^n$.
  \item Ein Weiteres Standardbeispiel\\
    \begin{center}
  ...
  \begin{tikzpicture}[opacity=0.7]
        \draw [thin,fill=lightgray] (0,0) to (1,0) to (0,1) to (0,0);
        \draw [thin,fill=lightgray] (0.5,0) to (1.5,0) to (0.5,1) to (0.5,0);
        \foreach \foo in {(0,0),(1,0),(0,1),(0.5,0),(1.5,0),(0.5,1)}
        \fill \foo circle[radius=0.07cm];
    \end{tikzpicture}
  \end{center}
\item Ein Komplex muss weder endlich noch als beschränkt sein.\\
    \begin{tikzpicture}
    ...
    Wobei der zweite Komplex die reelle Achse vollkommen ausfüllt
    bzw. darstellt.  Hierzu wird ein Begriff benötigt der in der
    folgenden Definition klar wird.
    
    % \draw [thin,fill=lightgray] (-0.7,0) to (0.7,0) to (-0.7,0);
    % \draw [thin,fill=lightgray] (90:1) to (210:1) to (330:1) to (90:1);
    % \fill (a,b) circle[radius=0.07cm];
    % \node (s) at (1,2) {$n=3$};
  ...
  \begin{enumerate}[(a)]
	...
	\begin{proof}
          ...
          \item Sei $v \in \Delta^{(0)}$ ein Knoten, so definiere
            folgende Abbildung
            % TODO: nutze die eigenschaft das für jeden punkt x \in
            % \gr{\Delta} genau ein simplex existiert in dessen
            % inneren der punkt liegt
  ...
  gelten folgende Aussagen
	\begin{enumerate}[(a)]
        \item $f(\Delta)$ ist ein Unterkomplex und das Urbild eines
          ...
          % TODO: ist das bild eines echten unterkomplexes wieder ein
          % unterkomplex, siehe trivial gegenbeispiele
